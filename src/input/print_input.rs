use crate::common::{
    Mode, ReadInputError, check_line, create_buf_write, create_read_buf, flush, map_file, write_all,
};
...
pub unsafe fn file_chunk_rayon(
    ...
    rayon::scope(move |scope| {
        for id in 0..num_workers {
            ...
            let mut end =  if id == num_workers - 1 {
                mmap_size
            } else {
                (id+1) * chunk_size
            };
            ...
            scope.spawn(move |_| {
                ...
                if end < mmap_size && id != num_workers{
                    if let Some(b) = memchr::memchr(b'\n', &mmap[end..]) {
                        end += b + 1;
                    } else {
                        end = mmap_size;
